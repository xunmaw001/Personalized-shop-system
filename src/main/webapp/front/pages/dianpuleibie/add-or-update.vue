	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`dianpuleibie`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`dianpuleibie`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`dianpuleibie`, this.ruleForm);
						}else{
							await this.$api.add(`dianpuleibie`, this.ruleForm);
						}

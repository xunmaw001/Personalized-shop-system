...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','dianpuleibie');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','dianpuleibie');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('dianpuleibie', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdianpuleibie/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('dianpuleibie', this.detail);
				this.$utils.msg('审核成功');
